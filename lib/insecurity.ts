...
// Supprimer la clé privée codée en dur : exiger une clé via l'environnement
const privateKey = process.env.JWT_PRIVATE_KEY ?? (() => { throw new Error('JWT_PRIVATE_KEY is required') })()
...
export const hmac = (data: string) => crypto.createHmac('sha256', String(process.env.HMAC_SECRET ?? 'change-me')).update(data).digest('hex')

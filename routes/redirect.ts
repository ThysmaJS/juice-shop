...
export function performRedirect () {
  return ({ query }: Request, res: Response, next: NextFunction) => {
    ...
    if (!toUrl) {
      res.status(400)
      return next(new Error('Missing target URL'))
    }
    if (security.isRedirectAllowed(toUrl)) {
      ...
      // Redirect using a trusted allowlist base to avoid echoing user input
      const matchedBase = security.redirectAllowlist.find((allowedUrl) => utils.startsWith(toUrl, allowedUrl))
      if (matchedBase) {
        return res.redirect(matchedBase)
      }
      res.status(406)

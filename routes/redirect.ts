...
export function performRedirect () {
  return ({ query }: Request, res: Response, next: NextFunction) => {
    ...
    if (!toUrl || typeof toUrl !== 'string') {
      res.status(400)
      return next(new Error('Missing or invalid redirect URL'))
    }
    
    // Limit URL length to prevent DoS
    if (toUrl.length > 2048) {
      res.status(400)
      return next(new Error('Redirect URL too long'))
    }
    ...
    // Use original security check but with validated input
